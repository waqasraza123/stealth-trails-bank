# Deposit Settlement Reconciliation and Replay API

## Purpose

This runbook covers the first operator-facing reconciliation and safe replay slice for deposit settlement.

This slice lets operators:

- inspect deposit settlement health
...
Endpoint:
GET /transaction-intents/internal/reconciliation/deposit-settlements?limit=20

Optional state filter:
GET /transaction-intents/internal/reconciliation/deposit-settlements?state=ready_for_settle_replay
...
- inspects deposit intents on the product chain in:
  - `broadcast`
  - `confirmed`
  - `settled`
...
Endpoint:
POST /transaction-intents/internal/reconciliation/deposit-settlements/:intentId/replay-confirm
...
Endpoint:
POST /transaction-intents/internal/reconciliation/deposit-settlements/:intentId/replay-settle
...
- clear operator visibility into deposit settlement health
